...
//					  console.log(lines);
				...
				newWords += "</p>";
				$("#textContainer").html(newWords).trigger("create");
				$("#noOfWords").html(numberOfWords);
				$("#textContainer").find("p").each(function(){
					  var lines = $(this).context.offsetHeight / 1.3;
					  console.log(lines);
				});
				return;
				paintingWordsTimer = setInterval(paintTheWord, 30);
//				$("#textContainer").width($(window).width());
//				$("#textContainer").height($(document).height())
//						.trigger('create');
				tenPercWidth = 0.1 * $("#readingText").width();
				// COUNTING THE LINE NUMBERS
				$("#textContainer p").each(function(j, t) {
					if ($(t).html().length <= 0) {
						$(t).remove();
						return false;
					}
